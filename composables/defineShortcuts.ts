...
// Helper functions to replace @vueuse/math
function logicNot(source: ComputedRef<boolean>): ComputedRef<boolean> {
  return computed(() => !source.value)
}

function logicAnd(...sources: (ComputedRef<boolean> | WatchSource<boolean>)[]): ComputedRef<boolean> {
  return computed(() => sources.every(source => 
